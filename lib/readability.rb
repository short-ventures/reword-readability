  ...
  class Document
    ...
    def title
      ...
      if cur_title =~ / [\|\-\\\/>»] /
        ...
        unless match
          cur_title = orig_title[(orig_title.rindex(':') + 1)..-1]

          if cur_title.split(/\s+/).size < 3
            cur_title = orig_title[(orig_title.index(':') + 1)..-1]

            if orig_title[0...orig_title.index(':')].split(/\s+/).size > 5
        ...
        if h_ones.size == 1
          cur_title = h_ones[0].text
        end
      end

      cur_title = cur_title.strip.gsub(/\s{2,}/, ' ')
      cur_title_word_count = cur_title.split(/\s+/).size

      if cur_title_word_count <= 4 && (!title_had_hierarchical_separators || cur_title_word_count != word_count(orig_title.gsub(/[\|\-\\\/>»]+/, '') - 1))
        cur_title = orig_title
      end
